...
const AdminServiceOnlinePatients = () => {
  ...
  const handleViewProfile = (row) => {
    if (row?.user_id) {
      navigate(`/admin/patient/${row.user_id}`);
      return;
    }
    alert('No linked user account for this booking');
  };
  ...
  const handleDownloadRecords = async (row) => {
    try {
      ...
      const recordsResponse = await axios.get(`/api/admin/medical-records/patient/${row.patient_id}`);
  ...
  const handleViewMedicalNotes = async (row) => {
    try {
      ...
      if (!bookingId) { setNotesLoading(false); return alert('Missing booking id'); }
      const res = await axios.get(`/api/admin/medical-record/${bookingId}`);
...
export default AdminServiceOnlinePatients;
