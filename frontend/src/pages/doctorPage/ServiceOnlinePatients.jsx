...
import { FaSearch, FaEye, FaFilePdf } from 'react-icons/fa';
...
const ServiceOnlinePatients = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError('');
      try {
        const response = await axios.get('/api/doctors/service-online-patients', {
  ...
  const handleViewProfile = (row) => {
    if (row?.user_id) {
      navigate(`/doctor/patient/${row.user_id}`);
      return;
    }
  ...
  const handleDownloadRecords = async (row) => {
    try {
      ...
      const recordsResponse = await axios.get(`/api/doctors/patient-medical-history/${row.patient_id}`);
...
export default ServiceOnlinePatients;
