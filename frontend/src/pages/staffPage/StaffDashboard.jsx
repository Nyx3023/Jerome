...
const StaffDashboard = () => {
  ...
  const [queueService, setQueueService] = useState('General Consultation')
  const [queueStatus, setQueueStatus] = useState({ waiting: 0, called: 0, estimated_wait_minutes: 0 })
  const [overdueAlerts, setOverdueAlerts] = useState([])
  const esRef = React.useRef(null);
  const reconnectTimerRef = React.useRef(null);
  const SHOW_QUEUE_BOARD = false;

  useEffect(() => {
    fetchAnalytics();
    initQueue();
    return () => {
      try { esRef.current && esRef.current.close(); } catch (e) { void e }
  ...
  const fetchQueueStatus = async (svc) => {
    try {
      const res = await axios.get('/api/staff/queues/status', { params: { service_type: svc } })
      setQueueStatus(res.data)
    } catch (e) { void e }
  }
  const initQueue = () => {
    fetchQueueStatus(queueService)
    try {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Staff Dashboard - Analytics</h1>
        <p className="text-gray-600 mt-1">Comprehensive insights into clinic operations</p>
      </div>
        ...
        <div className="border-b border-gray-200">
          <nav className="-mb-px flex space-x-8">
            {[
              ...
              { id: 'services', name: 'Services', icon: FaClipboardList },
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
            ...
            {SHOW_QUEUE_BOARD ? (
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="flex items-center gap-2 mb-4">
                  <select className="border rounded p-2" value={queueService} onChange={(e) => { setQueueService(e.target.value); fetchQueueStatus(e.target.value) }}>
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="bg-gray-50 rounded p-4">
                    ...
                    <div className="text-2xl font-bold">{queueStatus.called || 0}</div>
                  </div>
                  <div className="bg-gray-50 rounded p-4">
                    <div className="text-sm text-gray-600">Est. Wait (min)</div>
                    <div className="text-2xl font-bold">{queueStatus.estimated_wait_minutes || 0}</div>
                  </div>
                </div>
              </div>
            ) : null}
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            {/* Active Admissions (Quick View) */}
            <div className="bg-white rounded-lg shadow-md p-6 mt-6">
              <h3 className="text-lg font-semibold mb-4">Active Admissions</h3>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <div className="flex items-center justify-between">
                ...
                <div className="p-3 bg-purple-100 rounded-full">
                  <FaUserAlt className="w-6 h-6 text-purple-600" />
                </div>
              </div>
            </div>
          </div>
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const res = await axiosMod.default.get('/api/admin/admissions?status=admitted');
        setRows(res.data || []);
      } catch {
        setRows([]);
